...
@Component({
  selector: 'app-renewal-invoice',
  templateUrl: './renewal-invoice.component.html',
  styleUrls: ['./renewal-invoice.component.scss'],
})
export class RenewalInvoiceComponent {
  ...
  closeinvoicecount() {
    let url =
      ServerUrl.live +
      '/esim/getesimInvoiceCount?pagename=Renewal&status=Close';
    this.ajaxService.ajaxget(url).subscribe((res) => {
  ...
  deleteinvoicecount() {
    let url =
      ServerUrl.live +
      '/esim/getesimInvoiceCount?pagename=Renewal&status=Deleted';
    this.ajaxService.ajaxget(url).subscribe((res) => {
  ...
  gridchanges(value?) {
    if (this.activeTab.label == 'Pending') {
      ...
      let url =
        ServerUrl.live +
        '/esim/getAllRenewalEsimInvoice?userId=' +
        this.username+"&page="+this.gridPagination.currentPage;
        if(this.gridPagination.isSearched){
          url=url+"&Search="+this.gridPagination.searchTerm;
        }
      this.ajaxService.ajaxget(url).subscribe((res) => {
        this.invoicetabgrid = res;
        let countUrl="/esim/getAllCountRenewalEsimInvoice?userId="+this.username;
        if(this.gridPagination.isSearched){
  ...
  submit() {
    ...
    this.loader.presentLoader();
    let url = ServerUrl.live + '/esim/saveRenewalInvoiceDetails';

    this.ajaxService.ajaxPostWithBody(url, requestBody).subscribe((res) => {
      this.loader.dismissLoader();
      if (res.message == 'Renewal Amount has been Received Successfully') {
        this.messageService.add({
          key: 'toast1',
          severity: 'success',
          summary: 'Success',
  ...
  details(event) {
    ...
    let url =
      ServerUrl.live +
      '/esim/getRenewalInvoiceDetails?invoiceno=' +
