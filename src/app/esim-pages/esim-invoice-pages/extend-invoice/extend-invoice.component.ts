...
@Component({
  ...
  styleUrls: ['./extend-invoice.component.scss'],
})
export class ExtendInvoiceComponent {
  ...
  closeinvoicecount() {
    let url =
      ServerUrl.live + '/esim/getesimInvoiceCount?pagename=Extend&status=Close';
    this.ajaxService.ajaxget(url).subscribe((res) => {
  ...
  deleteinvoicecount() {
    let url =
      ServerUrl.live +
      '/esim/getesimInvoiceCount?pagename=Extend&status=Deleted';
    this.ajaxService.ajaxget(url).subscribe((res) => {
  ...
  gridchanges(value?) {
    if (this.activeTab.label == 'Pending') {
      ...
      let url =
        ServerUrl.live +
        '/esim/getinvoicestatus?pagename=Extend&status=' +
        this.whichgrid;
      this.ajaxService.ajaxget(url).subscribe((res) => {
        this.gridData = res;
      ...
      let url =
        ServerUrl.live +
        '/esim/getAllExtendOneYearEsimInvoice?userId=' +
      ...
      this.ajaxService.ajaxget(url).subscribe((res) => {
        ...
        let countUrl="/esim/getAllCountExtendOneYearEsimInvoice?userId="+this.username;
        if(this.gridPagination.isSearched){
  ...
  submit() {
    ...
    this.loader.presentLoader();
    let url = ServerUrl.live + '/esim/saveExtendOneYearInvoiceDetails';

    this.ajaxService.ajaxPostWithBody(url, requestBody).subscribe((res) => {
      this.loader.dismissLoader();
      if (
        res.message == 'Extend 1 Year Amount has been Received Successfully'
      ) {
        this.messageService.add({
          key: 'toast1',
          severity: 'success',
  ...
  export() {
    ...
    let reportData = {
      title: 'Extend One Year Invoice Details',
      data: forExcel,
      headers: Header,
    };
