...
@Component({
  selector: 'app-ca-invoice',
  templateUrl: './ca-invoice.component.html',
  styleUrls: ['./ca-invoice.component.scss'],
})
export class CaInvoiceComponent {
  ...
  gridchanges(value?) {
    if (this.activeTab.label == 'Pending') {
      ...
      this.loader.presentLoader();
      let url =
        ServerUrl.live + '/esim/getAllCAEsimInvoice?userId=' + this.username+"&page="+this.gridPagination.currentPage;
      ...
      this.ajaxService.ajaxget(url).subscribe((res) => {
        ...
        let countUrl="/esim/getAllCountCAEsimInvoice?userId="+this.username
        if(this.gridPagination.isSearched){
  ...
  submit() {
    ...
    this.loader.presentLoader();
    let url = ServerUrl.live + '/esim/saveCAInvoiceDetails';

    this.ajaxService.ajaxPostWithBody(url, requestBody).subscribe((res) => {
      this.loader.dismissLoader();
      if (res.message == 'CA Amount has been Received Successfully') {
        this.messageService.add({
          key: 'toast1',
          severity: 'success',
          summary: 'Success',
  ...
  details(event) {
    ...
    let url =
      ServerUrl.live +
      '/esim/getCAInvoiceDetails?invoiceno=' +
